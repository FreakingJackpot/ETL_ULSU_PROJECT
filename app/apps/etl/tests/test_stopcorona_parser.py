...
from apps.etl.utils.parsers import stopcorona_parser
...
class StopCoronaParserTestCase(TestCase):
    ...
    def test_get_url_list(self, mock_get):
        ...
        mock_response.text = """
              <body>
                ...
                <div>
                  <a class="u-material-card u-material-cards__card" href="v-rossii-za-nedelyu-vyzdoroveli-article4"></a>
                </div>
              </body>
          """

        expected_urls = ['v-rossii-za-nedelyu-vyzdorovelo-article1', 'v-rossii-za-nedelyu-vyzdorovelo-article2',
                         'v-rossii-za-nedelyu-vyzdorovelo-article3', 'v-rossii-za-nedelyu-vyzdoroveli-article4', ]
        urls = stopcorona_parser.StopCoronaParser._get_url_list(all=True)
        self.assertListEqual(urls, expected_urls)

        expected_urls = ['v-rossii-za-nedelyu-vyzdorovelo-article1', ]
        urls = stopcorona_parser.StopCoronaParser._get_url_list(all=False)
        self.assertListEqual(urls, expected_urls)

    @patch("apps.etl.utils.logging.Logger", LoggerMock)
    @patch("apps.etl.utils.parsers.stopcorona_parser.StopCoronaParser._get_url_list", Mock(return_value=None))
    def test_parse_page(self):
        ...
        for i in range(2):
            data = stopcorona_parser.StopCoronaParser()._parse_page(tests_data[i])
    ...
    def test_get_dates(self):
        ...
        for i in range(2):
            urls = stopcorona_parser.StopCoronaParser()._get_dates(soup[i])
    ...
    def test_clean_table_data(self):
        ...
        urls = stopcorona_parser.StopCoronaParser._clean_table_data(tests_data)
        self.assertListEqual(urls, expected_urls)
    ...
    def test_get_regions_data(self):
        ...
        urls = stopcorona_parser.StopCoronaParser._get_regions_data(tests_table_data, tests_dates)
        self.assertListEqual(urls, expected_urls)
    ...
    def test_parse_url_list(self, mock_get):
        ...
        urls = stopcorona_parser.StopCoronaParser()._parse_url_list([1])
