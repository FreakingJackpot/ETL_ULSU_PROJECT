...
class RosstatParser:
    ...
    def _get_year_url_pairs(cls, all):
        ...
        file_urls = (a['href'] for a in filter(lambda x: cls._media_url_path in x['href'], a_with_refs))
    ...
    def _parse_url_list(self, year_url_pairs):
        ...
        for year, url in year_url_pairs:
            ...
            if 'rar' in url:
                parsed_data.extend(self._get_data_from_rar(response.content))
            else:
                parsed_data.extend(self._get_data_from_xlsx(response.content))
    ...
    def _get_data_from_rar(self, bytes, year):
        filename_template = self._rar_xls_filename_template(year % 100)
    ...
    def _get_data_from_xlsx(self, bytes, year, one_sheet=False):
        read_excel = partial(pd.read_excel, bytes, skiprows=4, usecols='A,B', )
        df = read_excel() if one_sheet else read_excel(sheet_name=self._sheet_name)
